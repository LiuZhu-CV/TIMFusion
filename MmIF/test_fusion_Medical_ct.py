...
if __name__ == '__main__':
    ...
    genotype_1 = eval("genotypes.%s" % 'fusion_mri')
    genotype_2 = eval("genotypes.%s" % 'fusion_ct')
    ...
    for index in range(1,17):
        ...
        im_input = cv2.imread('../medical/MR-CT_3/mri_'+str(index)+'.png')
        ...
        ###############################################
        im_input2 = cv2.imread('../medical/MR-CT_3/ct_'+str(index)+'.png',0)
        ...
        with torch.no_grad():
            ...
            # ___,__,_, res = model(im_input, im_input2)
        ...
        # im_[:,:,0] =res[:,:,0]
        # res = adjust(res[:,:,0])
        if not os.path.exists('./our_mri_ct/'):
            os.mkdir('./our_mri_ct/')
        cv2.imwrite('./our_mri_ct/'+str(index)+'.png',res)
